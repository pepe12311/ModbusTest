...
labelContador = tk.Label(frame1, textvariable=contador, width=10, relief=tk.SUNKEN, bd=1)
labelContador.grid(column=2, row=6, sticky=tk.W + tk.E)
        ...
        for n in range(addr, min(addr + incremento, 65536)):
            ...
            try:
                if read_type == READ_COILS:
                    read=master.read_coils(address=n, count=1,slave=int(slave.get()))
                elif read_type == READ_DISCRETE_INPUTS:
                    read=master.read_discrete_inputs(address=n, count=1,slave=int(slave.get()))     
                elif read_type == READ_HOLDING_REGISTERS:
                     read=master.read_holding_registers(address=n, count=1,slave=int(slave.get()))
                elif read_type == READ_INPUT_REGISTERS:
                   read=master.read_input_registers(address=n, count=1,slave=int(slave.get()))
